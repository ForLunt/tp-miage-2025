...
Le code est structuré en couches car nous avons trois couche (base de donnée, code métier). Le problème est que l’application est dépendente de la thechnologie utilisé.
...
`todoItemRepository` est null car elle n'est pas initialisée. La meilleur facon de l'injecter est d'utilisé `@Autowired`
